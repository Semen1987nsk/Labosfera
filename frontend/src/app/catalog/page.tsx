...
async function getProducts(): Promise<Product[]> {
  const apiUrl = `${BACKEND_URL}/api/v1/products/`;
  try {
    const res = await fetch(apiUrl, { cache: 'no-store', headers: { 'Accept': 'application/json' } });
    if (!res.ok) { return []; }
    return res.json();
      ...
      {categories.length > 0 ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {categories.map((category) => (
            <Link key={category.id} href={`/catalog/${category.slug}`} className="block bg-dark-blue p-4 rounded-lg text-center transition-all duration-300 hover:bg-electric-blue/20 hover:-translate-y-1">
              <h3 className="font-semibold text-light-grey">{category.name}</h3>
            </Link>
          ))}
        </div>
      ) : (<p className="text-light-grey/70">Категории не найдены.</p>)}
      <h2 className="text-2xl font-semibold mt-16 mb-6">Популярные товары</h2>
      {products.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
          {products.slice(0, 8).map((product) => (<ProductCard key={product.id} product={product} />))}
        </div>
      ) : (<p className="text-light-grey/70">Товары не найдены.</p>)}
    </div>
  );
}